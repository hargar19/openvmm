...
pub fn prepare_kexec() -> anyhow::Result<()> {
    ...
    build_and_compress_initramfs(&binary_path, img_path).context("failed to build initramfs")?;

    let initramfs_size = std::fs::metadata(img_path).map(|m| m.len()).unwrap_or(0);

    tracing::info!(
        CVM_ALLOWED,
        initramfs_size,
        "using direct vmlinux kexec_file_load path"
    );
    prepare_kexec_vmlinux(img_path, &cmdline).context("direct vmlinux kexec path failed")?;
...
fn prepare_kexec_vmlinux(initrd_path: &str, cmdline: &str) -> anyhow::Result<()> {
    let kernel_file = std::fs::File::open(VMLINUX_IMAGE)
...
fn build_and_compress_initramfs(binary_path: &Path, output_path: &str) -> anyhow::Result<()> {
    ...
    let gz = flate2::GzBuilder::new().write(output_file, flate2::Compression::fast());
        ...
        for &(prefix, module) in KERNEL_MODULES {
            ...
            match std::fs::read(&src) {
                Ok(data) => {
                    ...
                    write_cpio_entry(&mut out, &mut inode, &name, S_IFREG | 0o644, 1, &data, 0, 0)?;
                }
                Err(e) => {
                    tracing::warn!(
                        CVM_ALLOWED,
